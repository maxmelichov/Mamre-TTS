...
from Mamre.backbone import BACKBONES
from Mamre.codebook_pattern import apply_delay_pattern, revert_delay_pattern
from Mamre.conditioning import PrefixConditioner
from Mamre.config import InferenceParams, MamreConfig
from Mamre.sampling import sample_from_logits
from Mamre.speaker_cloning import SpeakerEmbeddingLDA
from Mamre.utils import DEFAULT_DEVICE, find_multiple, pad_weight_

DEFAULT_BACKBONE_CLS = next(iter(BACKBONES.values()))


class Mamre(nn.Module):
    def __init__(self, config: MamreConfig, backbone_cls=DEFAULT_BACKBONE_CLS):
        super().__init__()
        self.config = config
        ...
        # Optional override for speaker embedding device in multi-GPU setups
    ...
    def _pad_embeddings_and_heads(self, *args, **kwargs):
        # Only pad embeddings; keep linear heads output size stable (1025)
    ...
    def from_local(
        ...
        config = MamreConfig.from_dict(json.load(open(config_path)))
        if backbone:
    ...
    def make_speaker_embedding(self, wav: torch.Tensor, sr: int) -> torch.Tensor:
        ...
        if self.spk_clone_model is None:
            device = self.speaker_device if self.speaker_device is not None else DEFAULT_DEVICE
            self.spk_clone_model = SpeakerEmbeddingLDA(device=device)
    ...
    def generate(
        ...
        prefix_conditioning = prefix_conditioning.to(device=device, dtype=model_dtype)
        ...
        with torch.device(device):
            inference_params = self.setup_cache(batch_size=batch_size * 2, max_seqlen=seq_len, dtype=model_dtype)
