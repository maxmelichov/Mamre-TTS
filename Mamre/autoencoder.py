...
class DACAutoencoder:
    ...
    def decode(self, codes: torch.Tensor) -> torch.Tensor:
        # Ensure codes are on the same device as the DAC model
        target_device = self.dac.device if hasattr(self.dac, "device") else next(self.dac.parameters()).device
        codes = codes.to(target_device)
        with torch.autocast(target_device.type, torch.float16, enabled=target_device.type != "cpu"):
