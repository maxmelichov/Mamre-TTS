...
from Mamre.config import PrefixConditionerConfig
...
supported_language_codes = [
    ...
    'vi-vn-x-central', 'vi-vn-x-south', 'yue', 'he'
]  # fmt: off
...
def make_cond_dict_train(
    text: Union[str, List[str]] = "It would be nice to have time for testing, indeed.",
    language: Union[str, List[str]] = "en-us",
    speaker: torch.Tensor | None = None,
    ...
    assert language.lower() in supported_language_codes or (isinstance(language, (list, tuple)) and all(l.lower() in supported_language_codes for l in language)), "Please pick a supported language"
    
    language_code_to_id = {lang: i for i, lang in enumerate(supported_language_codes)}

    # If speaker is not provided, generate a random one.
    if speaker is None:
    ...
    # Check if text is already a list/tuple.
    if isinstance(text, (list, tuple)):
        text_list = list(text)
    else:
        text_list = [text]

    # Check if language is already a list/tuple.
    if isinstance(language, (list, tuple)):
        language_list = list(language)
    else:
        language_list = [language]

    # If only one language is provided but multiple texts, replicate the language.
    if len(language_list) == 1 and len(text_list) > 1:
        language_list = language_list * len(text_list)

    cond_dict = {
        "espeak": (text_list, language_list),
        "speaker": speaker,
        "emotion": emotion,
        ...
        "language_id": torch.tensor([language_code_to_id[language_list[0]]], device=device),
        "vqscore_8": vqscore_8,
